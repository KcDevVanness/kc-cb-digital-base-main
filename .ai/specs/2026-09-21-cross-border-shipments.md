...
Verdict: `Implemented` (Phases A–C, smoke-verified 2026-09-21; assumptions Q1–Q3 remain open to owner override).

> **As-shipped deltas (2026-09-23).** The verdict line above contradicted the Status line and was corrected.
> Two facts the body never learned:
> 1. `cross_border_shipments` gained `container_type`, `container_number`, `seal_number`, `booking_number`
>    (`Migration20260922082558_cross_border.ts`), and `EXPORT_DOC_TYPES` gained `so`, `telex_release`,
>    `domestic_freight_receipt`, `booking_charges_receipt` (容器事实与四类新单证，2026-09-22).
> 2. Allocations now refuse a purchase-order line **without an official catalog product link** (422, message
>    "… has no catalog product link, so the goods cannot be received into stock"), in addition to the
>    quantity guard — see [`.ai/lessons/stock-receipt-needs-variant-resolution.md`](../lessons/stock-receipt-needs-variant-resolution.md).
> **Test evidence:** no automated tests exist under `src/modules/cross_border/**`; the acceptance evidence is
> the smoke recorded in the plan (阶段三).
...
| 2026-09-23 | Fixed the verdict line that contradicted the Status; added as-shipped deltas: container facts (4 columns + 4 document types, `Migration20260922082558`) and the catalog-link allocation guard; test evidence recorded as manual smoke. |
