...
**Date**: 2026-09-23
**Status**: Draft — **Phase 0 shipped 2026-09-23** (`storage_s3` installed, wired, credentialed-by-config and probed against MinIO; both partitions still `storage_driver='local'`); Phase 1 awaits the go-ahead
...
- **Tests:** TEST-004 reused against a pruned partition (rollback must refuse once local files are gone, naming the reason)
- **Validation:** the broad gate; `yarn test:integration` (MinIO)
- **Exit gate:** `prune-local --dry-run` lists exactly the verified rows; `--yes` removes only those files; `rollback` afterwards fails closed with a readable reason; docs match the tree.
...
- [ ] **AC-007** — `rollback` restores the partition row verbatim from the pre-flip snapshot, strips the prefix, brings back rows written after the cutover, and is byte-identical; it fails closed with a readable reason when an object is unreadable or local files were pruned. (REQ-005)
